#!/bin/sh
# mysql_innodb_cluster_migrate.sh v1.0.45
...
Usage: sh $0 discover|capabilities|sql-precheck|strict-gtid|gr-restart-precheck|configure-admin|precheck|preflight|configure|plan|create|adopt|validate|status|all
  ...
  preflight [all|clone|tls|xcom] Check Clone capacity/platform, TLS and GR communication; read-only
...
# Preflight runs in a subshell so its helper variables cannot alter migration state.
# Authentication, temporary credentials and locking are owned by this same script.
preflight_help(){ cat <<EOF
Usage: sh $0 preflight [all|clone|tls|xcom]
Run discover first with the same MYSQL_IC_WORK_ROOT.
Environment:
  MYSQL_IC_CLONE_DONOR                  registered donor node number (default 1)
  MYSQL_IC_CLONE_SPACE_MARGIN_PERCENT   capacity margin (default 10)
  MYSQL_IC_TLS_MODE                     AUTO|DISABLED|REQUIRED|VERIFY_CA|VERIFY_IDENTITY
  MYSQL_IC_TLS_CA                       readable CA file on the execution host
  MYSQL_IC_COMMUNICATION_STACK          AUTO|XCOM|MYSQL
This is read-only for MySQL; local evidence is written under MYSQL_IC_WORK_ROOT.
Manual checks and warnings are reported separately. Clone version compatibility
is finally checked by AdminAPI at addInstance(), not by a hard-coded version table.
EOF
}
preflight() (
    trap - 0 1 2 15
    pf_step=${1:-all}
    case $pf_step in all|clone|tls|xcom|communication) :;; *) preflight_help; die "Unknown preflight command: $pf_step";; esac
    require_discovery_schema
    [ -f "$ROOT/meta/complete" ] || die 'Run discover first'
    assert_identity
    MANUAL_CHECKS=0
...
summary(){
    ...
    case $pf_step in
        all) clone_precheck; tls_precheck; xcom_precheck;;
        clone) clone_precheck;;
        tls) tls_precheck;;
        xcom|communication) xcom_precheck;;
    esac
    summary
)

# Help does not require clients, discovery metadata or a work-root lock.
case $STEP in
    help|-h|--help) help; exit 0;;
    preflight)
        case ${2:-all} in
            help|-h|--help) preflight_help; exit 0;;
            all|clone|tls|xcom|communication) :;;
            *) preflight_help; die "Unknown preflight command: $2";;
        esac;;
esac
TMP=''
LOCK=''
...
# Only record ownership after successfully acquiring the lock.
mkdir "$ROOT/.lock" 2>/dev/null || die "Another operation is using $ROOT"
LOCK="$ROOT/.lock"
TMP=$(mktemp -d "$ROOT/.tmp.XXXXXX")
need "$MYSQL"; need "$MYSQLSH"
case $STEP in help|-h|--help) help;; discover) discover;; capabilities) show_capabilities;; sql-precheck) sql_precheck;; strict-gtid) require_discovery_schema; [ -f "$ROOT/meta/complete" ] || die 'Run discover first'; assert_identity; strict_exact_gtid_guard; log 'STRICT GTID CHECK PASSED';; gr-restart-precheck) gr_restart_precheck;; configure-admin) configure_admin;; precheck) precheck;; preflight) preflight "${2:-all}";; configure) configure;; plan) plan;; create) create;; adopt) adopt;; validate) validate;; status) status;; all) all;; *) help; die "Unknown command: $STEP";; esac
