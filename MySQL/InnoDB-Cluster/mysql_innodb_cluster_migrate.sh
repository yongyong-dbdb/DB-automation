#!/bin/sh
# mysql_innodb_cluster_migrate.sh v1.0.22
...
EOF
    ...
    cidr=$(ip -4 route show dev "$iface" scope link 2>/dev/null | awk -v h="$first" '$1 ~ /^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+\/[0-9]+$/ {print $1; exit}')
    [ -n "$cidr" ] || { printf ''; return 0; }
    printf '%s' "$cidr"
}
