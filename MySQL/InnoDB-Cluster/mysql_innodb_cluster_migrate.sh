#!/bin/sh
# mysql_innodb_cluster_migrate.sh v1.0.25
...
register_node(){
    i=$1
    mode=$(ask_explained "Node $i authentication method" "How this script authenticates to the instance during discovery/bootstrap." "login-path | password" "Authentication method for Node $i" "password")
    case $mode in
        login-path)
