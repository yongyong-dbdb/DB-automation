...
assert_one() {
    n=$(grep -F -c "$1" "$TARGET" || true)
    [ "$n" -eq 1 ] || { echo "anchor count for [$1] = $n" >&2; exit 1; }
}

assert_one 'REMOTE_RECEIVER_SLOT=""'
awk '{print} $0=="REMOTE_RECEIVER_SLOT=\"\"" {print "REMOTE_MAX_WAL_SENDERS=\"\""; print "REMOTE_MAX_REPLICATION_SLOTS=\"\""; print "REMOTE_REPLICATION_SLOT_COUNT=\"\""; print "REMOTE_RISKY_PHYSICAL_SLOT_COUNT=\"\""; print "REMOTE_ARCHIVED_COUNT=\"\""; print "REMOTE_ARCHIVER_FAILED_COUNT=\"\""; print "REMOTE_LAST_ARCHIVED_TIME=\"\""; print "REMOTE_LAST_FAILED_TIME=\"\""; print "REMOTE_ARCHIVER_UNRESOLVED=\"\""}' "$TARGET" > "$TARGET.tmp" && mv "$TARGET.tmp" "$TARGET"

cat > /tmp/controller_funcs <<'EOF'
...
EOF
assert_one 'startup_options_guard() {'
awk 'FNR==NR {ins=ins $0 ORS; next} /^startup_options_guard\(\) \{/ {printf "%s", ins} {print}' /tmp/controller_funcs "$TARGET" > "$TARGET.tmp" && mv "$TARGET.tmp" "$TARGET"

cat > /tmp/remote_func <<'EOF'
...
EOF
assert_one 'remote_wait_streaming_downstreams() {'
awk 'FNR==NR {ins=ins $0 ORS; next} /^remote_wait_streaming_downstreams\(\) \{/ {printf "%s", ins} {print}' /tmp/remote_func "$TARGET" > "$TARGET.tmp" && mv "$TARGET.tmp" "$TARGET"

assert_one '    --remote-wait-streaming-downstreams)'
awk '/^    --remote-wait-streaming-downstreams\)/ {print "    --remote-switchover-safety)"; print "        [ \"$#\" -eq 2 ] || exit 64"; print "        remote_switchover_safety \"$2\""; print "        exit $?"; print "        ;;"} {print}' "$TARGET" > "$TARGET.tmp" && mv "$TARGET.tmp" "$TARGET"

assert_one '    acquire_candidate_lock'
awk '{print} $0=="    acquire_candidate_lock" {print "    current_primary_slot_wal_status_guard"; print "    candidate_switchover_safety_precheck 0"; print "    external_restart_fencing_guard"}' "$TARGET" > "$TARGET.tmp" && mv "$TARGET.tmp" "$TARGET"

assert_one '    LOCAL_ARCHIVE_READY=$(archive_mechanism_configured 2>/dev/null || echo 0)'
awk '{print} $0=="    LOCAL_ARCHIVE_READY=$(archive_mechanism_configured 2>/dev/null || echo 0)" {print "    local_archiver_health_guard"; print "    candidate_archiver_health_guard"}' "$TARGET" > "$TARGET.tmp" && mv "$TARGET.tmp" "$TARGET"

assert_one '        record_check "MANUAL CHECK" "Continuous Archiving" "destination and credentials confirmed by operator"'
awk '$0=="        record_check \"MANUAL CHECK\" \"Continuous Archiving\" \"destination and credentials confirmed by operator\"" {print "        if ! choose_yes_no \"Shared archive destination의 동일 WAL 파일 중복 처리 정책(멱등 저장/동일 내용 재전송)이 안전함을 확인했습니까\" \"no\"; then"; print "            die \"Continuous archiving duplicate-WAL handling was not confirmed.\""; print "        fi"; print "        record_check \"MANUAL CHECK\" \"Archive Duplicate WAL Handling\" \"operator confirmed duplicate WAL handling is safe\""} {print}' "$TARGET" > "$TARGET.tmp" && mv "$TARGET.tmp" "$TARGET"

assert_one '    startup_options_guard || die "Switchover cannot guarantee that the former Primary will restart with the same startup configuration."'
awk '$0=="    startup_options_guard || die \"Switchover cannot guarantee that the former Primary will restart with the same startup configuration.\"" {print "    if [ -n \"${REVERSE_SLOT:-}\" ]; then"; print "        candidate_switchover_safety_precheck 1"; print "    else"; print "        candidate_switchover_safety_precheck 0"; print "    fi"} {print}' "$TARGET" > "$TARGET.tmp" && mv "$TARGET.tmp" "$TARGET"

assert_one '                warn "Keep the former Primary stopped until it is confirmed to start with standby.signal and correct primary_conninfo."'
awk '{print} $0=="                warn \"Keep the former Primary stopped until it is confirmed to start with standby.signal and correct primary_conninfo.\"" {print "                manual_recovery_branch_notice"}' "$TARGET" > "$TARGET.tmp" && mv "$TARGET.tmp" "$TARGET"

sh -n "$TARGET"
grep -F 'candidate_switchover_safety_precheck 0' "$TARGET" >/dev/null
grep -F 'candidate_switchover_safety_precheck 1' "$TARGET" >/dev/null
grep -F "wal_status IN ('unreserved','lost')" "$TARGET" >/dev/null
grep -F 'pg_stat_archiver' "$TARGET" >/dev/null
grep -F 'manual_recovery_branch_notice' "$TARGET" >/dev/null
grep -F 'External HA / Service Restart / Fencing' "$TARGET" >/dev/null
grep -F -- '--remote-switchover-safety' "$TARGET" >/dev/null
...
git add "$TARGET"
git rm '.github/workflows/patch-postgresql-role-switch-safety.yml' '.github/patch-postgresql-role-switch-safety.sh'
git commit -m 'fix: harden PostgreSQL planned switchover safety checks'
