...
' "${REMOTE_SYNC_STANDBY_NAMES:-<empty>}"
    ...
    kv "pg_stat_activity.backend_type=client backend" "$active_sessions"
    kv "pg_stat_activity.xact_start IS NOT NULL" "$active_tx"
    ...
    current_direct_counts=$(psql_call "SELECT count(*)::text || E'\\t' || count(*) FILTER (WHERE state='streaming')::text FROM pg_stat_replication r WHERE state <> 'backup' AND NOT EXISTS (SELECT 1 FROM pg_replication_slots s WHERE s.active_pid=r.pid AND s.slot_type='logical')" 2>/dev/null | sed -n '1p') || current_direct_counts=""
    current_direct_total=$(printf '%s\n' "$current_direct_counts" | awk -F '\t' '{print $1}')
    current_direct_streaming=$(printf '%s\n' "$current_direct_counts" | awk -F '\t' '{print $2}')
    if [ "$current_direct_total" != "$DIRECT_STANDBY_COUNT" ] || [ "$current_direct_streaming" != "$DIRECT_STANDBY_COUNT" ]; then
        rollback_preconfigured_primary
        die "pg_stat_replication changed before Primary Server shutdown: total=${current_direct_total:-unknown}, state=streaming=${current_direct_streaming:-unknown}, expected=$DIRECT_STANDBY_COUNT."
    fi
    if ! remote_invoke --remote-validate-candidate "$REMOTE_PGDATA" "$SYSTEM_IDENTIFIER" "$PGPORT" "${CANDIDATE_SLOT:-}" "$REMOTE_DOWNSTREAM_COUNT" >/dev/null; then
        rollback_preconfigured_primary
        die "The selected Standby Server's Current Role, system_identifier, pg_stat_wal_receiver, recovery settings, or pg_stat_replication result changed before Primary Server shutdown."
    fi
