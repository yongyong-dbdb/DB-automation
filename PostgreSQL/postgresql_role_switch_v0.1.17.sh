...
record_check() {
    ...
    printf '%s\t%s\t%s\n' "$rc_status" "$rc_name" "$rc_detail" >> "$RESULT_FILE" 2>/dev/null || true
}
...
finalize_result_report() {
    ...
    if [ "$frr_code" -eq 0 ]; then
        frr_status="PASSED"
        frr_detail="operation completed"
    ...
    printf '%s\t%s\t%s\n' "$frr_status" "Final Result" "$(redact_result_text "$frr_detail")" >> "$RESULT_FILE" 2>/dev/null || true
}
...
acquire_instance_lock() {
    ail_uid=$(id -u 2>/dev/null || echo unknown)
    ail_base=${XDG_RUNTIME_DIR:-${TMPDIR:-/tmp}}
    LOCK_ROOT="$ail_base/postgresql-role-switch-$ail_uid"
    [ ! -L "$LOCK_ROOT" ] || die "Refusing symbolic-link lock root: $LOCK_ROOT"
    if [ ! -d "$LOCK_ROOT" ]; then
        mkdir "$LOCK_ROOT" 2>/dev/null || die "Could not create lock directory: $LOCK_ROOT"
    fi
    chmod 700 "$LOCK_ROOT" 2>/dev/null || true

    ail_key=$(printf '%s|%s' "$SYSTEM_IDENTIFIER" "$PGDATA" | cksum 2>/dev/null | awk '{print $1}')
    [ -n "$ail_key" ] || ail_key=$(printf '%s_%s' "$SYSTEM_IDENTIFIER" "$PGDATA" | sed 's/[^0-9A-Za-z]/_/g')
    LOCK_DIR="$LOCK_ROOT/$ail_key.lock"
...
check_pg_wal_free_space() {
    ...
    if [ "$cpw_available_bytes" -lt "$cpw_required_bytes" ]; then
        ...
        record_check "PASSED" "pg_wal Free Space" "available_bytes=$cpw_available_bytes, required_bytes=$cpw_required_bytes"
    fi
}
...
check_wal_retention_risk() {
    cwr_operation=$1
    check_pg_wal_free_space
    record_check "MANUAL CHECK" "$cwr_operation" "max_wal_size is not a hard WAL retention limit; assess WAL generation rate, physical replication slots and outage duration before continuing"
...
select_primary_candidate() {
    ...
    while :; do
        spc_idx=$(ask "Standby Server number" "") || die "Input cancelled."
        case "$spc_idx" in
...
remote_select_instance() {
    ...
    rsi_preflight_line=$(remote_invoke --remote-preflight "$REMOTE_PGDATA" 2>/dev/null | awk -F '\t' '$1=="PREFLIGHT" {print; exit}') || rsi_preflight_line=""
    ...
    if [ -n "$rsi_available_bytes" ] && [ -n "$rsi_required_bytes" ] && [ "$rsi_available_bytes" -lt "$rsi_required_bytes" ] 2>/dev/null; then
        ...
        record_check "PASSED" "Standby Server pg_wal Free Space" "available_bytes=${rsi_available_bytes:-unknown}, required_bytes=${rsi_required_bytes:-unknown}"
    fi
    record_check "PASSED" "Remote Execution Environment" "data_directory=$REMOTE_PGDATA, port=$REMOTE_PORT"
}

verify_upstream_connection() {
    vuc_result=$(remote_invoke --remote-verify-upstream "$REMOTE_PGDATA" "$SYSTEM_IDENTIFIER" "$PGPORT" 2>/dev/null) || remote_die "Could not connect from the selected Standby Server to its configured upstream Primary Server. Check primary_conninfo, credentials, pg_hba.conf and network."
...
candidate_downstream_check() {
    ...
    remote_invoke --remote-downstreams "$REMOTE_PGDATA" > "$cdc_tmp" 2>/dev/null || die "Could not inspect the selected Standby Server's downstream Standby Servers."
...
unselected_standby_check() {
    usc_file=$(mktemp_safe) || die "Could not create a temporary pg_stat_replication file."
    ...
    while [ "$usc_i" -le "$usc_count" ]; do
        ...
        usc_host=$(ask "Standby Server SSH host or 'local'" "$usc_client") || die "Input cancelled."
        [ -n "$usc_host" ] || die "Standby Server host is required."
        if [ "$usc_host" = "local" ]; then
            usc_transport="local"
            usc_target="local"
        else
            command -v ssh >/dev/null 2>&1 || die "ssh client is not available for Standby Server verification."
            usc_user=$(ask "Standby Server SSH user" "$(id -un 2>/dev/null || echo '')") || die "Input cancelled."
            [ -n "$usc_user" ] || die "Standby Server SSH user is required."
            usc_transport="ssh"
            usc_target="$usc_user@$usc_host"
            ssh -o BatchMode=yes -o ConnectTimeout=5 "$usc_target" 'sh -c "exit 0"' >/dev/null 2>&1 || die "Non-interactive SSH check failed for $usc_target."
        fi

        usc_instances=$(mktemp_safe) || die "Could not create a Standby Server instance file."
        invoke_on_transport "$usc_transport" "$usc_target" --remote-list-downstream "$SYSTEM_IDENTIFIER" "$PGPORT" "$usc_slot" > "$usc_instances" 2>/dev/null || die "Could not discover the matching PostgreSQL instance on $usc_host."
        ...
        if [ "$usc_matches" -eq 1 ]; then
            ...
            usc_pick=$(ask "Standby Server instance number for $usc_app" "") || die "Input cancelled."
        fi
        case "$usc_pick" in ''|*[!0-9]*) die "Invalid Standby Server instance selection." ;; esac
        usc_selected=$(awk -F '\t' -v n="$usc_pick" '$1=="DOWNSTREAM_INSTANCE" {i++; if(i==n) print}' "$usc_instances")
        [ -n "$usc_selected" ] || die "Invalid Standby Server instance selection."
...
' "${REMOTE_SYNC_STANDBY_NAMES:-<empty>}"
    ...
    validate_wait_seconds "$catchup_wait" || die "Wait seconds must be an integer between 1 and $MAX_WAIT_SECONDS."
...
remote_verify_upstream() {
    ...
    rvu_conninfo=$(psql_call "SHOW primary_conninfo" 2>/dev/null | sed -n '1p') || return 1
    [ -n "$rvu_conninfo" ] || return 1
    # psql uses the same libpq connection string as the WAL receiver. Never
    # print or store it: it may contain a password or passfile location.
    ...
    rvu_observed=$(PGCONNECT_TIMEOUT="${PGCONNECT_TIMEOUT:-10}" "$PSQL_BIN" -X -q -A -t -v ON_ERROR_STOP=1 -d "$rvu_dsn" -c "SELECT system_identifier::text || E'\\t' || current_setting('port') || E'\\t' || pg_is_in_recovery()::text FROM pg_control_system()" 2>/dev/null | sed -n '1p') || return 1
