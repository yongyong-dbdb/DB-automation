...
psql_call_var() {
    ...
    if [ -n "$PGUSER_LOCAL" ] && [ -n "$SOCKET_DIR" ] && [ -n "$PGPORT" ]; then
        ...
        printf '%s\n' "$pcv_sql" | "$PSQL_BIN" -X -q -A -t -v ON_ERROR_STOP=1 -v "$pcv_name=$pcv_value" -U "$PGUSER_LOCAL" -p "$PGPORT" -d "$DB_NAME"
    elif [ -n "$SOCKET_DIR" ] && [ -n "$PGPORT" ]; then
        printf '%s\n' "$pcv_sql" | "$PSQL_BIN" -X -q -A -t -v ON_ERROR_STOP=1 -v "$pcv_name=$pcv_value" -h "$SOCKET_DIR" -p "$PGPORT" -d "$DB_NAME"
    elif [ -n "$PGPORT" ]; then
        printf '%s\n' "$pcv_sql" | "$PSQL_BIN" -X -q -A -t -v ON_ERROR_STOP=1 -v "$pcv_name=$pcv_value" -p "$PGPORT" -d "$DB_NAME"
    else
...
normalize_downstream_relation_set() {
    ndrs_file=$1
...
' "${REMOTE_SYNC_STANDBY_NAMES:-<empty>}"
    ...
    if [ "$REPARENT_UNSELECTED" -ne 1 ] 2>/dev/null; then
    ...
    printf '  SELECT count(*) FROM pg_stat_replication: %s (effective application_name=%s, slot=%s)\n' "$new_primary_sees_old" "$OLD_PRIMARY_EFFECTIVE_APP" "${REVERSE_SLOT:-<none>}"
...
review_inactive_physical_slots() {
    ...
    while :; do
        ...
        if [ "$LOCAL_ROLE" = "primary" ]; then
            ...
            case "$mm_choice" in
                ...
                2) replication_status; review_inactive_physical_slots ;;
                3) exit 0 ;;
                *) say "Invalid selection." ;;
            esac
        else
            ...
            case "$mm_choice" in
                ...
                2) manual_failover ;;
                3) replication_status; review_inactive_physical_slots ;;
                4) exit 0 ;;
                *) say "Invalid selection." ;;
            esac
        fi
...
remote_count_reparented() {
    ...
    rcr_count=$(psql_call_var3 app "$rcr_app" client "$rcr_client" slot "$rcr_slot" "SELECT count(*) FROM pg_stat_replication r WHERE state='streaming' AND application_name=:'app' AND COALESCE(host(client_addr),'local')=:'client' AND (CASE WHEN :'slot'='' THEN NOT EXISTS (SELECT 1 FROM pg_replication_slots s WHERE s.active_pid=r.pid AND s.slot_type='physical') ELSE EXISTS (SELECT 1 FROM pg_replication_slots s WHERE s.active_pid=r.pid AND s.slot_type='physical' AND s.slot_name=:'slot') END)" 2>/dev/null | tr -d '[:space:]') || exit 3
...
remote_create_slot() {
    ...
    case "$rcs_state" in
        absent) psql_call_var slot "$rcs_slot" "SELECT slot_name FROM pg_create_physical_replication_slot(:'slot', true, false)" >/dev/null || exit 5 ;;
        physical_inactive) : ;;
