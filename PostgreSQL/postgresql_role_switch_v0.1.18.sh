...
remote_verify_upstream() {
    ...
    # Reuse WAL receiver libpq parameters without placing credentials in the
    # psql argv (which can be exposed by process listings). The environment
    # is still visible to the same OS account, so prefer passfile/cert auth.
    ...
    rvu_observed=$(PGDATABASE="$rvu_dsn" PGCONNECT_TIMEOUT="${PGCONNECT_TIMEOUT:-10}" "$PSQL_BIN" -X -q -A -t -v ON_ERROR_STOP=1 -c "SELECT system_identifier::text || E'\\t' || current_setting('port') || E'\\t' || pg_is_in_recovery()::text FROM pg_control_system()" 2>/dev/null | sed -n '1p') || return 1
